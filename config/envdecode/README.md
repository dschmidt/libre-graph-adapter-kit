> Vendored into libre-graph-adapter-kit verbatim from
> [opencloud-eu/opencloud](https://github.com/opencloud-eu/opencloud)
> (`pkg/config/envdecode`), so adapter services can bind env vars to their
> config structs without depending on the opencloud module. Licensed MIT
> (see LICENSE); upstream is itself a fork of joeshaw/envdecode.
